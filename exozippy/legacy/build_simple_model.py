...
# EXOZIPPy imports
from .jointfit.inputs import parse_priors, read_rv_data, read_transit_data
from .sed.utils import mistmultised, read_sed_file
from ..physics.exozippy_tran import exozippy_tran
...
def build_simple_model(parfile=None,
    ...
    if parfile is not None:
        user_params = parse_priors(parfile)
    else:
        user_params = {}
    ...
    if tranpath is not None:
        ...
        for tranfile in tranfiles:
            transit_data.append(read_transit_data(tranfile))
    ...
    if rvpath is not None:
        ...
        for rvfile in rvfiles:
            rv_data.append(read_rv_data(rvfile))
    ...
    def get_param_value(param_name, default_value, params_dict=user_params):
        ...
        if param_name in params_dict:
            # Support both legacy (mu/initval) and EXOFAST-style (value) prior dictionaries.
            if 'value' in params_dict[param_name] and params_dict[param_name]['value'] is not None:
                return params_dict[param_name]['value']
        ...
        try:
            ...
            if len(transit_data) > 0:
                ...
                for i, tdata in enumerate(transit_data):
                    ...
                    flux_err = tdata.get('flux_err', tdata.get('err'))
                    if flux_err is not None:
                        residuals = (tdata['flux'] - model_flux) / flux_err
                    else:
            ...
            if len(rv_data) > 0:
                ...
                for i, rdata in enumerate(rv_data):
                    ...
                    # Calculate RV model
                    rv_obs = rdata.get('rv', rdata.get('vel'))
                    rv_err = rdata.get('rv_err', rdata.get('err'))
                    model_rv = exozippy_rv(rdata['bjd'], tc, period, gamma, K)
                    
                    # Calculate chi-squared with overflow protection
                    if rv_err is not None:
                        residuals = (rv_obs - model_rv) / rv_err
                    else:
                        # Use 1 m/s error as default if no errors provided
                        residuals = (rv_obs - model_rv) / (1.0 * np.ones_like(rv_obs))
