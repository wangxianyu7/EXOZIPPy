...
Usage:
    from exozippy.plotting.transit import plottran
...
from exozippy.jointfit import read_transit_data
from exozippy.jointfit.chi2 import tc_to_tp, derive_ar as _derive_ar
from .common import DEFAULT_COLORS, rounded_symmetric_limit, save_or_show, short_label
...
def _oc_ylim(residuals):
    ...
    return rounded_symmetric_limit(residuals, fallback=0.001)


def _transit_label(filepath):
    """Extract a short label from a transit file path."""
    return short_label(filepath)
...
def plottran(tranfiles, bestfit, samples=None, mstar=None,
    ...
    save_or_show(fig, outfile, label='transit')
